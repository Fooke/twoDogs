...
class Hx extends Model {
    ...
    private $client_id='YXA6gP4BoLfFEeeC3IHAi5wjow';
    private $client_secret='	YXA6OpQO3ETZ1hsN8Oy4qjasM80P05A';
    private $org_name='1116171009115865';
    private $app_name='	twodogs';
    private $url = 'https://a1.easemob.com/1116171009115865/twodogs/';
    ...
    public function openRegister($user_id) {
        $User = User::get($user_id);
        $password = $User->password;
        $options['username'] = $User->phoneNumber;
     ...
     */
    public function editPassword($user_id,$newPassword) {
        $User = User::get($user_id);
        $options = array();
        $options['username'] = $User->phoneNumber;
    ...
    public function deleteUser($user_id) {
        ...
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $info = $this->postCurl ( $url, '', $header, $type = 'DELETE' );
        if(isset($info['entities'])){
            return '1';
        }else{
            return '0';
     ...
     *          $friend_id    好友id
     */
    public function addFriend($owner_id, $friend_id) {
        $owner = User::get($owner_id);
        $friend = User::get($friend_id);
        $url = $this->url . "users/" . $owner->phoneNumber . "/contacts/users/" . $friend->phoneNumber;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
     ...
     *          $friend_id    好友id
     */
    public function deleteFriend($owner_id, $friend_id) {
      $owner = User::get($owner_id);
      $friend = User::get($friend_id);
        $url = $this->url . "users/" . $owner->phoneNumber . "/contacts/users/" . $friend->phoneNumber;
        $access_token = $this->getToken ();
     ...
     *          $owner_id   主人id
     */
    public function showFriend($owner_id) {
        $url = $this->url . "users/" . $owner_id . "/contacts/users/";
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, '', $header, $type = "GET" );
    ...
    public function isOnline($user_id) {
        $user  = User::get($user_id);
        $url = $this->url . "users/" . $user->phoneNumber . "/status";
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, '', $header, $type = "GET" );
        $result = json_decode($result);
     ...
     */
     ...
     public function hxSend($send_id, $accept_id, $content, $from_user = "admin", $target_type = "users") {
      $sendUser = User::get($send_id);
      $acceptUser = User::get($accept_id);
      $sendMap = array($sendUser->phoneNumber, $acceptUser->phoneNumber);
        $option ['target_type'] = $target_type;
        $option ['target'] = $sendMap;
        $params ['type'] = "txt";
        $params ['msg'] = $content;
        $option ['msg'] = $params;
     ...
     */
    public function createGroups($room_id, $owner_id, $desc, $public = true, $approval = false, $member = array()) {
        $owner = User::get($owner_id);
        $option['groupname'] = (string)$room_id;
        $option['owner'] = (string)$owner_id;
        $option['desc'] = $desc;
        $option['public'] = $public;
        $option['approval'] = $approval;
        $option['members'] = $member;
        $url = $this->url . "chatgroups";
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, $option, $header );
        $info = json_decode($result,true);      //将json数据转换为数组
        return  $info['data']['groupid'];       //返回IM群组id
     ...
     *          $user_id    用户id
     */
    public function addGroupsUser($group_id, $user_id) {
        $user = User::get($user_id);
        $url = $this->url . "chatgroups/" . $group_id . "/users/" . $user_id;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
     ...
     *          $user_id    用户id
     */
    public function delGroupsUser($group_id, $user_id) {
        $user = User::get($user_id);
        $url = $this->url . "chatgroups/" . $group_id . "/users/" . $user->phoneNumber;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, '', $header, $type = "DELETE" );
        return $result;
